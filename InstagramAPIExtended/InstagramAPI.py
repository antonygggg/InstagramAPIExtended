...
class InstagramAPI:
    ...
    def getStoryViewers(self, story_pk, maxid = None):
        request = 'media/'+str(story_pk)+'/list_reel_media_viewer/'
        if(maxid >= 0):
            request = request + '&max_id=' + maxid
    ...
    def getSelfRecentMedia(self):
        activity = self.SendRequest('/users/'+str(self.username_id)+'/media/recent/?access_token=' + self.token)
        return activity
    ...
    def block(self, userId):
        data = json.dumps({'_uuid': self.uuid,
                           '_uid': self.username_id,
                           'user_id': usercurrent_follow_requestsId,
                           '_csrftoken': self.token})
